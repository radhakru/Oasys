...
public class ReadCamsMarketingFertilizerExcel {
	...
	public Object[][] getFile() throws IOException {
		...
		fis = new FileInputStream(System.getProperty("user.dir") + "//Externaldata//Sale_Of_Fertilizer.xlsx");
		...
		for (int i = rowvalue; i <= rowsize; i++) {
			for (int j = 0; j < 16; j++) {
				if (j == 5 || j == 6 || j == 7 || j == 8 || j == 9 || j == 10 || j == 11 || j == 12 || j == 13
						|| j == 14 || j == 15) {
...
//		// return this
	...
	public void update_cell_value(int rownum, String text) throws IOException {
		...
		System.out.println("String cell value for ..." + xssfSheet.getRow(rownum).getCell(15).getCellType());
		...
		if (xssfSheet.getRow(rownum).getCell(13).getCellType().equals(CellType.NUMERIC)
				|| xssfSheet.getRow(rownum).getCell(16).getCellType().equals(CellType.BLANK)
				|| xssfSheet.getRow(rownum).getCell(16).getCellType().equals(CellType.STRING)) {
			xssfSheet.getRow(rownum).createCell(16).setCellValue(text);
			fos = new FileOutputStream(System.getProperty("user.dir") + "//Externaldata//Sale_Of_Fertilizer.xlsx");
